...
class SFluxTest extends FreeSpec with Matchers with TableDrivenPropertyChecks with TestSupport with IdiomaticMockito {
  "SFlux" - {
    ...
    ".firstEmitter" - {
      "with varargs of publisher should create Flux based on the publisher that emit first onNext or onComplete or onError" in {
        val flux: SFlux[Long] = SFlux.firstEmitter(SMono.delay(Duration("10 seconds")), Mono.just[Long](1L))
        StepVerifier.create(flux)
          .expectNext(1)
          .verifyComplete()
      }
    ...
    ".onBackpressureError" in {
      val jFlux = spy(JFlux.just(1, 2, 3), true)
      val flux = SFlux.fromPublisher(jFlux)
